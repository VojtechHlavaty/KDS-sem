...
int send_ack(uint32_t packet_num) {
    ...
    client_addr.sin_port = htons(SENDING_PORT);
    if (sendto(data_sockfd, &ack, sizeof(ack), 0, 
...
int send_nack(uint32_t packet_num) {
    ...
    if (sendto(data_sockfd, &nack, sizeof(nack), 0, 
               (struct sockaddr *)&client_addr, client_len) < 0) {
...
int receive_packet(Packet *packet) {
    while (1) {
        ...
        if (packet->packet_number != expected_packet) {
            ...
            // Still send ACK to avoid sender retransmission
            send_ack(packet->packet_number);  
            continue;
        }

        // Send ACK for the correct (in-sequence) packet
...
/*
 * This function encapsulates the main receiving loop logic. 
 * It opens the output file once the filename packet arrives,
 * stores the MD5 hash, and writes the file data until termination.
 */
static int handle_reception() {
    ...
    // Calculate the MD5 of the received file and compare
